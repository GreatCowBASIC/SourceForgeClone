...
''Namespace Great_Cow_Graphical_BASIC
	...
	Public Class ErrorListBox
		...
		Private Dim ErrorLocations As List(Of String)
		...
		Public Sub New(CurrFile As GCBProgram, ErrorFile As String)
			...
			ErrorLocations = New List(Of String)
		...
		Public Sub Reload
			...
			Do While ErrorReader.Peek() <> -1
				TempData = ErrorReader.ReadLine().Replace(Convert.ToChar(9), " ").Trim
				
				ResultSection = TempData.Split("|".ToCharArray, 5)
				...
				If ResultSection.Length = 5 Then
					...
					If CurrFile.Filename.EndsWith("\" + ResultSection(1)) Or CurrFile.Filename.EndsWith("/" + ResultSection(1)) Or ResultSection(1).ToLower = "gcgb.tmp" Then
						...
						Catch
						End Try
						If SubNo < 0 Or SubNo >= CurrFile.Subroutines.Count Then
							SubName = "Unknown"
						Else
