...
Namespace Great_Cow_Graphical_BASIC
	...
	Public Class GCBProgram
		...
		Public Dim ProgInterrupts As ArrayList
		...
		Private Dim LocalConstGroups As ArrayList
		...
		'Undo/Redo History
		Public Dim UndoRedoHistory(MainForm.MaxUndo) As System.Collections.ArrayList
		Public Dim UndoRedoCount As Integer
		Public Dim UndoRedoLocation As Integer
		
		'Library data storage arrays
		Public Dim Libraries As ArrayList
		...
		Public Sub New(Editor As EditorPanel)
			...
			ProgInterrupts = New ArrayList
			Constants = New ArrayList
			LocalConstGroups = New ArrayList
		...
		Public Function GetTextProgram As String
	    	...
	    	Dim Temp, InnerTemp, SubNo, CurrentIndent, NewIndent, CurrLineNo As Integer
		...
		Public Sub ReadTextProgram(InData As String, Optional SourceFile As String = "")
	    	...
	    	Dim ProcessTemp, SplitLineTemp As String
	    	Dim SpaceCount, EqLoc As Integer
	    	...
	    	Dim Temp As Integer
	    	Dim EnableChange As Integer
	    	
	    	Dim ChipModel As String
	    	Dim DefineTemp As String
	    	Dim LabelTemp As String = ""
	    	Dim NewVarName, NewVarType As String
	    	Dim CodeFromLine As ArrayList
		...
		Public Sub InsertLine(ByVal CurrentSub As GCBSubroutine, ByVal NewProgLine As String, AddIndex As Integer)
			...
			If GetCalledSub(NewProgLine) Is CurrentSub And Preferences.GetPref("GCGB", "WarnRecursion") = "1" Then
		...
		Public Function GetCommandData(ByVal InputLine As String) As String
			...
			Dim CommandPointer, IsMatch, IndexTemp, Temp As Integer
		...
		Public Function GetLabels(CurrSub As GCBSubroutine) As ArrayList
		...
		Public Function GetAllConstGroups As ArrayList
		...
		Public Sub LoadIncludes
			...
			Dim DefineName, DefineValue, ChipModel As String
			...
			For Each LoadLib In Libraries
				FileNameTemp = LoadLib.FileName
				If FileNameTemp.IndexOf("<") <> -1 Then
	   				FileNameTemp = FileNameTemp.Replace("<", "").Replace(">", "").Trim
	   				FileNameTemp = MainForm.InstallDir + "\include\" + FileNameTemp
		   				...
		   				Do While ReadInclude.Peek() <> -1
							...
							Else If TempData.ToLower.StartsWith("#chip ") And LoadLib.ChipName = "" Then
			   				...
			   				Else If TempData.ToLower.StartsWith("dim ") And Not InSub Then
			   					Dim NewVar As New VariableListItem
			   					NewVar.Code = TempData
		...
		Public Function ReadDirective(ByVal TempData As String, Optional ByRef CurrDoc As DocSection = Nothing) As Boolean
			...
			Dim NewVarName, NewVarType As String
			Dim Temp, FoundIndex As Integer
		...
		Public Sub FillParamList(ByVal CurrentSub As GCBSubroutine, ByRef ParamListBox As System.Windows.Forms.ComboBox, ByVal ParamList As String, ByVal CurrentValue As String, Optional ParamName As String = "")
			Dim Temp, AddListItem, AddParams, OptionCount, LibTemp As Integer
			...
			Do While ParamList.IndexOf("|") <> -1
				OptionCount += 1
				OptionList(OptionCount) = ParamList.Substring(0, ParamList.IndexOf("|")).Trim.ToLower
			...
			For AddListItem = 1 To OptionCount
				...
				Dim VarTypes() As String = {"byte", "word", "integer", "string"}
				...
				If OptionList(AddListItem).StartsWith("range") Or OptionList(AddListItem).StartsWith("const") Or _
				   ...
				   OptionList(AddListItem) = "number" Then
					'Set min and max
		...
		Public Function GetParamString(ByVal InputLine As String, ByVal SyntaxTemp As String) As ArrayList
