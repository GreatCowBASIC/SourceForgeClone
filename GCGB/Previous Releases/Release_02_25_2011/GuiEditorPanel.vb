...
Namespace Great_Cow_Graphical_BASIC
	...
	Public Class GuiEditorPanel
		...
		Public Sub New()
			...
			If Preferences.GetPref("GCGB", "Background") <> "" Then
				Try
					Me.ProgEditPanel.BackgroundImage = Image.FromFile(LowLevel.ReplaceToolVariables(Preferences.GetPref("GCGB", "Background")))
				Catch
				End Try
			End If
			...
			CurrentSub = 0
		 	...
		 	If Not MainForm.PrefGroups Is Nothing Then
			 	Dim SettingsPanelWidth, SubListHeight As String
			 	SettingsPanelWidth = Preferences.GetPref("GCGB", "SettingsPanelWidth")
			 	SubListHeight = Preferences.GetPref("GCGB", "SubListHeight")
...
"tine")
		...
		Private Function ShiftKeyDown As Boolean
			...
			If (Control.ModifierKeys.Shift And Control.ModifierKeys) <> 0 Then Return True
			Return False
		End Function
		
		Private Function CtrlKeyDown As Boolean
			If (Control.ModifierKeys.Control And Control.ModifierKeys) <> 0 Then Return True
			Return False
		End Function
		...
		Private Sub ProgramRedraw
			...
			Dim CurrentLine, IndentCount, TextTemp As Integer
			...
			Dim NameParams As New ArrayList
		...
		Private Sub DrawCommand(ByVal DataTemp As String, CurrentLine As Integer)
			Dim IconName, ProcessTemp as String
			Dim CommandTemp, SyntaxTemp, CommandName As String
			Dim CommandPointer, IndexTemp, IsMatch As Integer
			Dim ParamStart, ParamData, ParamEnd, CommandRemainder As String
			Dim IconToolTip As String
			Dim ParamNo As Integer
			Dim IconParameters As New ArrayList
		...
		Private Sub DrawSubList
			...
			Dim SubListData As New ArrayList
		...
		Private Sub DrawIcon(ByVal IconName As String, ParamList As ArrayList, Pos As Integer, ByVal Indent As Integer, Optional IconToolTip As String = "")
			...
			If Not CurrIcon Is Nothing Then
				...
				CurrentIconPlace = IconSpacing(Pos)
			 	If Indent < 100 Then IconSpacing(Pos + 1) = CurrentIconPlace + 20 + CurrIcon.YSize
			 	...
			 	With Me.IconDisplay(Pos)
				 	...
				 	.Location = New System.Drawing.Point(Indent * 20 + (200 - CurrIcon.XSize) / 2, 20 + CurrentIconPlace)
		...
		Private Sub DrawParameters
			...
			Dim CommandIndex, AddListItem As Integer
			Dim CGR_Name, CGR_Value As String
			Dim LibTemp, TempData, AddParams As Integer
			'Dim TextOnly As Boolean = False
			Dim ParamMode As Integer = 0
			Dim Temp As Integer = 0
			Dim CommandParams As ArrayList
			...
			Case -1
				...
				Dim NameTemp, ValueTemp, TypeTemp As String
		...
		Private Sub ParameterChanged
			...
			If NewCommand <> "" Then
				...
				Dim LastEventList As ArrayList = Program.UndoRedoHistory(Program.UndoRedoLocation)
		...
		Private Sub AddUndo(ByVal NewEvent As UndoHistoryItem, Optional AddToCurrent As Boolean = False)
			...
			With Program
				...
				Else
					...
					.UndoRedoHistory(.UndoRedoCount) = New System.Collections.ArrayList
		...
		Private Sub ProgEditPanelMouseMove(sender As System.Object, e As System.Windows.Forms.MouseEventArgs)
			Dim MousePos, FindLocation, MouseOverIcon, FindMatch, RedrawStart, OldLocation As Integer
		...
		Private Sub AddIcon(ByVal DraggedCommand As String, ByVal MouseOverIcon As Integer)
			Dim FindLocation, FindMatch, RedrawStart, OldLocation As Integer
			Dim IsName As Boolean = False
		...
		Private Sub FormSplitterSplitterMoved(sender As System.Object, e As System.Windows.Forms.SplitterEventArgs)
			Preferences.SetPref("GCGB", "SettingsPanelWidth", formSplitter.SplitPosition)
			Preferences.SavePreferences
			
		End Sub
		
		Private Sub RightPaneSplitterSplitterMoved(sender As System.Object, e As System.Windows.Forms.SplitterEventArgs)
			Preferences.SetPref("GCGB", "SubListHeight", Me.RightPaneSplitter.SplitPosition)
			Preferences.SavePreferences
