...
The analog to digital converter (ADC or A/D) module support is implemented by Great Cow BASIC to provide 8-bit, 10-bit and 12-bit Single channel measurement mode and Differential Channel Measurement with support up to 34 channels.

For compatibility all channels are supported.

To mimise the code, use the following to disable support for a channel AD0.

----
		#define USE_AD0 FALSE
----

The following tables show the `#define`s that can be used to reduce the code size - these are the defines for the standard microcontrollers.
For 16f1688x devices please see the second table.
...
This is a example - disables every channel except the specified channel by defineing every channel except USE_AD0 as FALSE.
