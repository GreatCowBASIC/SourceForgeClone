...
'    Copyright Evan R. Venn 2019-2020
'
...
'
'    11 bytes are used as input and output parameters.
'    They are:
'       DS_Value, DS_Addr, DS_Hour, DS_Min, DS_Sec, DS_EnableState
'       DS_Date, DS_Month, DS_Year and DS_DOW.
'
'    Quick Command Reference:
'
'    PCF85x3_Enable( boolean_flag )
'    PCF85x3_ResetClock
...
'    PCF85x3_WriteRegister( Register, value )
'    PCF85x3_ReadRegister( Register, out_value )
...
'    PCF85x3_SetClockout( boolean_flag )
'    PCF85x3_SetClockoutFrequency( clock_frequency_constant )
'
...
;----- Constants
 ...
 #define DS_AddrRead  0xA3
  ...
  #define  PCF85x3_WKDAY_REG_MASK                  0x07
...
sub PCF85x3_SetAlarm(in DS_Hour, in DS_Min, in DS_Date, in DS_DOW )
...
sub PCF85x3_SetInterrupt( in DS_Enableval as byte = true )


  dim DS_EnableState as byte
     ...
     do
        ...
        HI2CSend(DS_AddrWrite)                        ;indicate a write
     loop While HI2CAckPollState
     HI2CSend(PCF85x3_CTRL_REG_2)                     ;indicate register
     HI2CReStart
     HI2CSend(DS_AddrRead)
     HI2CReceive(DS_EnableState, NACK)                ;get theregiser
     if DS_Enableval.1 then
      set DS_EnableState.1 on              ;enables
     else
      set DS_EnableState.1 off             ;disables
     end if
     HI2CReStart
     HI2CSend(DS_AddrWrite)
     HI2CSend(PCF85x3_CTRL_REG_2)                     ;indicate register
     ...
     I2CStart
     I2CSend(DS_AddrWrite)
     I2CSend( PCF85x3_CTRL_REG_2 )                     ;indicate register 0
     I2CStart
     I2CSend(DS_AddrRead)
     I2CReceive(DS_EnableState, NACK)       ;get the current seconds
     if DS_Enableval.1 then
...
sub PCF85x3_ReadClock(out DS_Hour, out DS_Min, out DS_Sec, out DS_Date, out DS_DOW, out DS_Month, out DS_Year)
...
;-----
'Ported
sub PCF85x3_ReadDate( out DS_Date, out DS_DOW, out DS_Month, out DS_Year)
...
Function PCF85x3_ReadAlarmState
     ...
     do
        ...
        HI2CSend(DS_AddrWrite)                     ;inidcate a write
     loop While HI2CAckPollState
    HI2CSend( PCF85x3_CTRL_REG_2 )                      ;begin
    HI2CReStart
    HI2CSend(DS_AddrRead)
    HI2CReceive(DS_Sec, NACK)
    PCF85x3_ReadAlarmState = BcdToDec(DS_Sec & 8 ) ;strip off alarm bit
    HI2CStop
    ...
    I2CStart
    I2CSend(DS_AddrWrite)
    I2CSend( PCF85x3_CTRL_REG_2 )                      ;begin
    I2CStart
    I2CSend(DS_AddrRead)
    I2CReceive(DS_Sec, NACK)
    PCF85x3_ReadAlarmState = BcdToDec(DS_Sec & 8 ) ;strip off alarm bit
    I2CStop
...
sub PCF85x3_SetClockout ( optional in DS_Value  = true )
...
sub PCF85x3_SetClockoutFrequency(in DS_Value)
