...
'    Copyright (C) 2019-2020 Giuseppe D'Elia and Evan Venn
...
'  Initial release
'  12.12.2019      Add SPIRAM support
'  14.12.2019      Changed variables names as these clashed with other use variables.  Major test failure resolved,
...
sub   UpdatePageData_EPD2in13D

    Dim EPD_Ind_raw, EPD_Ind_col as long
    ...
    for EPD_Ind_raw=1 to BUFFWIDTH #IGNOREWARNING
       'replaced with macro for speed
        ...
        #ifdef SPISRAM_TYPE
          'SRAMRead ( [long]EPD_Ind_raw, [byte]EPD2in13D_Data )  'The subroutine method
          EPD2in13D_Data = SRAMRead ( EPD_Ind_raw)
        #endif
        #ifndef SPISRAM_TYPE
...
Sub DrawChar_EPD2in13D(In CharLocX as word, In CharLocY as word, In CharCode, Optional In LineColour as word = GLCDForeground )
  ...
  dim oldEPDGLCDBackground, oldEPDGLCDForeground as word
  dim LineColourToggled as bit
  ...
  'You can make independent change to section 2 and 3 but they are mutual exclusive with many common pieces
    ...
    if LineColour <> GLCDForeground  then
      'Inverted Colours
      oldEPDGLCDBackground = GLCDBackground
      oldEPDGLCDForeground = GLCDForeground
      GLCDForeground = LineColour
      GLCDBackground = oldEPDGLCDForeground
      LineColourToggled = 1
    else
      LineColourToggled = 0
    end if
  ...
  'CharCode needs to have 15 subtracted, table starts at char 16 not char 0
   ...
   CharCol=0
    ...
    if LineColourToggled = 1  then
      GLCDForeground = oldEPDGLCDForeground
      GLCDBackground = oldEPDGLCDBackground
    end if
...
Sub PSet_EPD2in13D(In GLCDX as word, In GLCDY as word, In GLCDColour As Word)
  ...
  Dim EPD_Ind, EPD_N_Col, EPD_N_raw as word
  Dim  EPD_Ind_Trans as long
...
'    EPD_N_Col=(GLCDX+1)/8                   'number of the column of the byte to be set
    ...
    #ifdef SPISRAM_TYPE
      'SRAMRead ( [long]EPD_Ind_Trans, [byte]DataVar )    'The subroutine method
      DataVar = SRAMRead ( EPD_Ind_Trans )
    #endif
    #ifndef SPISRAM_TYPE
    ...
    #ifdef SPISRAM_TYPE
      SRAMWrite ( [long]EPD_Ind_Trans, [byte]DataVar  )
    #endif
    #ifndef SPISRAM_TYPE
