...
'''  #define MAX_COL_PIXEL       240   'MAX_COL_PIXEL the physical matrix width (x direction)    This is specific to the GLCD display
    ...
    #script
        ...
        'calculated page size.  Not sure why we need the extra 24 pixels but we do on the 240 * 64.
        GLCDPAGESIZE = INT( MAX_COL_PIXEL  * ( MAX_ROW_PIXEL + 24 ) / 8 )
        GLCDPage0_T6963 = GRH_HOME_ADDR
        GLCDPage1_T6963 = GLCDPage0_T6963 + GLCDPAGESIZE
        GLCDPage2_T6963 = GLCDPage1_T6963 + GLCDPAGESIZE
        GLCDPage3_T6963 = GLCDPage2_T6963 + GLCDPAGESIZE
        GLCDPage4_T6963 = GLCDPage3_T6963 + GLCDPAGESIZE
        GLCDPage5_T6963 = GLCDPage4_T6963 + GLCDPAGESIZE
        GLCDPage6_T6963 = GLCDPage5_T6963 + GLCDPAGESIZE
        GLCDPage7_T6963 = GLCDPage6_T6963 + GLCDPAGESIZE
...
'''Sub to send a value of the byte to the device
'''@param addr value of the memory location for GLCD current page
Sub SelectGLCDPage_T6963 ( in addr as word )
    dim current_grh_home_addr as word
...
'''Sub to send a value of the byte to the device
'''@param addr value of the memory location for LCD current page
Sub SelectLCDPage_T6963 ( in addr as word )
    dim current_text_home_addr as word
...
'''Sub to send a value of the byte to specific LCD screen position
...
Sub GLCDispAutoDataWr_T6963   ( In LCDDataByte )
