...
Dim Shared As Integer ChipPins, UseChipOutLatches, AutoContextSave, TinyBootloaderSet, ChipIO, ChipADC
...
SUB CalcConfig
  ...
  If ChipFamily <> 16 Then
    ...
    'Do not set CONFIG is TBL+
    If TinyBootloaderSet = 0 Then
        'Write code
        'Single config word
        If ConfWords = 1 Then
  ...
  If ModePIC Then
    ...
    If ChipFamily = 12 Or ChipFamily = 14 Or ChipFamily = 15 Then
      If HasSFR("INTCON") Then
        ...
        if TinyBootloaderSet = -1 then
          CurrLine = LinkedListInsert(CurrLine, " movlw 0")
          CurrLine = LinkedListInsert(CurrLine, " movwf PCLATH")
        End if
...
Sub ReadOptions(OptionsIn As String)
  ...
  Do While CurrElement <> 0
    ...
    'Enable TinyBootloader Support
    ElseIf CurrElement->Value = "TBL" Then
      TinyBootloaderSet = -1
