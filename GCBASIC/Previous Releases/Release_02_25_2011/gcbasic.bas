...
Declare Function GetSubParam(ParamIn As String) As SubParam
...
Declare Sub AddVar(VarNameIn As String, VarTypeIn As String, VarSizeIn As Integer, VarSubIn As SubType Pointer, VarPointerIn As String, OriginIn As String)
...
Declare Sub MakeSFR (UserVar As String, SFRName As String)
...
Declare Sub GetTokens(InData As String, OutArray() As String, ByRef OutSize As Integer, DivChar As String = "")
...
Declare Function ReplaceToolVariables(InData As String, FNExtension As String = "", FileNameIn As String = "") As String
...
Dim SHARED SysVars(1000, 1 TO 2) As String
DIM SHARED SysVarBits(8000, 1 TO 3) As String '1 - name, 2 - location, 3 - var
...
Dim Shared CalcAtAsm(500) As String: CAAC = 0
...
Dim Shared ProgMemPage(100) As ProgMemPageType: ProgMemPages = 0
...
Dim As Integer MainProgramSize, CD, T, PD
...
IF Not ErrorsFound THEN
	...
	Else
		...
		PRINT SPC(10); Message("Vars") + Str(FVLC) + " (" + Str(ChipRam) + " max)"
		PRINT SPC(10); Message("Consts") + Str(DFC) + " (400 max)"
		PRINT SPC(10); Message("Subs") + Str(SBC) + " (500 max)"
		PRINT SPC(5); Message("AssemblyWritten") + Str(APC) + " (20000 max)"
	END IF
...
IF PrgExe <> "" AND AsmExe <> "" AND Not ErrorsFound THEN
	...
	PrgExe = ReplaceToolVariables(PrgExe, "hex")
	PrgParams = ReplaceToolVariables(PrgParams, "hex")
	...
	Do While CurrLine <> 0
		...
		IF INSTR(TempData, "movff") = 0 AND INSTR(TempData, "lfsr") = 0 AND INSTR(TempData, "retfie") = 0 AND INSTR(TempData, ",access") = 0 AND INSTR(TempData, ", access") = 0 AND TempData <> "" AND Left(TempData, 1) <> ";" THEN
			...
			IF INSTR(TempData, "F") <> 0 THEN
				...
				ConstFound = 0
				If HasSFR(VarName) Then ConstFound = 1
				If IsRegister(VarName) Then ConstFound = 1
				'Add banking mode
				IF ConstFound = 0 THEN CurrLine->Value = CurrLine->Value + ",BANKED"
				IF ConstFound = 1 THEN CurrLine->Value = CurrLine->Value + ",ACCESS"
			END IF
		END If
			...
			IF INSTR(OtherData, "F") <> 0 And WholeINSTR(TempData, "lfsr") <> 2 And WholeINSTR(TempData, "movff") <> 2 THEN
				...
				IF ChipFamily <> 16 Then
					For FindVar = 1 TO SVC
						If WholeINSTR(TempData, SysVars(FindVar, 1)) = 2 Then
							Bank = Val(SysVars(FindVar, 2)) And BankMask
							VarInBank = SysVars(FindVar, 1)
							Exit For
						End If
					Next
				End If
	...
	Do While CurrLine <> 0
		...
		If Left(CurrLine->Value, 6) = " call " Then
			...
			Do While GotoSub -> HasFinalGoto
				'Print "Call in "; CompSub->Name; " (page "; CompSub->DestPage; ")"
				'Print Chr(9) + "Goto from "; GotoSub->Name; " (page "; GotoSub->DestPage; ")"
				GotoSub = Subroutine(LocationOfSub(GotoSub->FinalGotoDest, ""))
				'Print Chr(9) + "Goto to "; GotoSub->Name; " (page "; GotoSub->DestPage; ")"
...
Sub AddSysIntCode
	...
	RegItems += 1: RegItem(RegItems) = "SysCalcTempA"
	...
	RegItems += 1: RegItem(RegItems) = "SysCalcTempA_H"
	...
	RegItems += 1: RegItem(RegItems) = "SysCalcTempA_U"
	RegItems += 1: RegItem(RegItems) = "SysDivMultA"
	RegItems += 1: RegItem(RegItems) = "SysStringA"
	RegItems += 1: RegItem(RegItems) = "SysCalcTempA_E"
	RegItems += 1: RegItem(RegItems) = "SysDivMultA_H"
	RegItems += 1: RegItem(RegItems) = "SysStringA_H"
	RegItems += 1: RegItem(RegItems) = "SysCalcTempB"
	RegItems += 1: RegItem(RegItems) = "SysCalcTempB_H"
	RegItems += 1: RegItem(RegItems) = "SysCalcTempB_U"
	RegItems += 1: RegItem(RegItems) = "SysDivMultB"
	RegItems += 1: RegItem(RegItems) = "SysReadA"
	RegItems += 1: RegItem(RegItems) = "SysCalcTempB_E"
			...
			Do While CurrCalled <> 0
				...
				Do While CurrLine <> 0
					...
					If ModePIC Then
						...
						'Always need to save PCLATH (25/11/2009)
						'ElseIf WholeINSTR(CurrLine->Value, "pagesel") <> 0 Then
	...
	If ModePIC Then
		...
		Subroutine(IntSubLoc)->FirstPage = -1
		'Interrupt sub doesn't need return added
		Subroutine(IntSubLoc)->NoReturn = -1
		
		'Get code start
		SubStart = Subroutine(IntSubLoc)->CodeStart
...
SUB CalcConfig
	...
	'Don't try this on AVR, it can't be done!
	If ModeAVR Then Exit Sub
...
SearchForOpAgain:
	...
	Else
		...
		If Act = "&" Or Act = "|" Or Act = "#" Or Act = "!" Then Answer = CompileCalcLogic(OutList, V1, Act, V2, Origin, AnswerIn)
	End If
...
Function CalcLineSize(CurrLine As String, ThisSubPage As Integer, CallPos As Integer, GotoPos As Integer, NextCodeLine As String) As Integer
	...
	If CallPos = -1 Then
		'Only PIC will do something different based on this
		If ModePIC Then
			CallPos = IsASM("call")
		Else
			CallPos = -999
	...
	Else
		'Need special treatment for call
		If InstIndex = CallPos Or InstIndex = GotoPos Then
			
			'Check if next line is call
			'(Decide if page needs to be restored after this call) 
			RestorePage = -1
			If NextCodeLine <> "" Then
				If Left(NextCodeLine, 6) = " call " Then
					GetTokens(NextCodeLine, TempData(), TempDataCount)
					NextCallTarget = TempData(2)
					NextCalledSub = LocationOfSub(NextCallTarget, "")
					NextCalledSubPage = Subroutine(NextCalledSub)->DestPage
					If NextCalledSubPage <> ThisSubPage Then
			...
			If ModePIC Then
				If ChipFamily = 12 Or ChipFamily = 14 Or ChipFamily = 15 Then
					GetTokens(CurrLineVal, TempData(), TempDataCount)
					CalledSub = LocationOfSub(TempData(2), "")
					CalledSubPage = Subroutine(CalledSub)->DestPage
					
					'If called sub ends in goto, restore
					If InstIndex = GotoPos Then
						'Never restore after Goto
						RestorePage = 0
						
					Else
						'Is call in different page? If not, don't restore
						If CalledSubPage = ThisSubPage Then
							RestorePage = 0
						End If
						
						'If call is to sub that ends in goto, maybe need to restore
						GotoSub = Subroutine(NextCalledSub)
						Do While GotoSub -> HasFinalGoto
							If ThisSubPage <> GotoSub->DestPage Then
								RestorePage = -1
								Exit Do
							End If
							GotoSub = Subroutine(LocationOfSub(GotoSub->FinalGotoDest, ""))
					...
					Else
						'enhanced 16F have movlp instruction, pagesel takes 1 word
						If ChipFamily = 15 Then
							InstSize = AsmCommands(InstIndex).Words + 1
						Else
							InstSize = AsmCommands(InstIndex).Words + 2
						End If
					End If
...
FUNCTION CompileCalcAdd(OutList As CodeSection Pointer, V1 As String, Act As String, V2 As String, Origin As String, Answer As String) As String
	...
	Dim As String V1O, V2O, V1Type, V2Type, CalcType, DestType, AV, R1, R2, AVH, Temp
	Dim As Integer SourceSub, DestSub
	Dim As String Cmd, Ovr, TempVar, V1H, V1RType, V2RType
	Dim As Integer CD
	...
	'Get real types of variables, having removed casts
	V1RType = TypeOfValue(V1, Subroutine(SourceSub))
	V2RType = TypeOfValue(V2, Subroutine(SourceSub))
	V1O = V1: V2O = V2
	'Print V1, V1Type, Act, V2, V2Type
	
	'Get output var
	If DestType = CalcType Or (DestType = "BYTE" And CalcType <> "SINGLE") Then
		AV = Answer
	Else
	...
	If Act = "+" And IsConst(V1) And (CalcType = "BYTE" Or CalcType = "WORD" Or CalcType = "INTEGER") Then
		'Swap names
		Swap V1, V2
		Swap V1O, V2O
	...
	'Put parameters into registers (AVR)
	If ModeAVR Then
		'Overwrite V1?
		Ovr = "O:"
		...
		NewCode = LinkedListCreate 
		R1 = PutInRegister(NewCode, Ovr + V1, CalcType, Origin)
		AddVar V1, "BYTE", 1, Subroutine(SourceSub), "REAL", Origin
		CurrLine = LinkedListInsertList(CurrLine, NewCode)
		'Put V2 into register, keep name in R2
		NewCode = LinkedListCreate
		R2 = PutInRegister(NewCode, V2, CalcType, Origin)
		AddVar V2, "BYTE", 1, Subroutine(SourceSub), "REAL", Origin
		CurrLine = LinkedListInsertList(CurrLine, NewCode)
	End If
	
	'Byte/low Word/low Integer
	If CalcType = "BYTE" Or CalcType = "WORD" Or CalcType = "INTEGER" Then
		If ModePIC Then
			'Check if V1 is a constant, and put into variable if it is
			IF IsConst(V1) And ChipFamily = 12 THEN
				CurrLine = LinkedListInsert(CurrLine, " movlw " + GetByte(V1, 0))
				V1 = GetCalcVar("BYTE")
				CurrLine = LinkedListInsert(CurrLine, " movwf " + V1)
				FreeCalcVar V1
			END IF
			
			'Move V2 into W
			Cmd = " movf " + V2 + ",W": IF IsConst(V2) THEN Cmd = " movlw " + GetByte(V2, 0)
			CurrLine = LinkedListInsert(CurrLine, Cmd)
			IF NOT IsConst(V2) THEN AddVar V2, "BYTE", 1, 0, "REAL", Origin
			...
			Else
				Cmd = " addwf ": IF Act = "-" THEN Cmd = " subwf "
				If V1 <> AV Then
					CurrLine = LinkedListInsert(CurrLine, Cmd + V1 + ",W")
					CurrLine = LinkedListInsert(CurrLine, " movwf " + AV)
				Else
					CurrLine = LinkedListInsert(CurrLine, Cmd + V1 + ",F")
				End If
			End If
	...
	'High Byte
	If (CalcType = "WORD" Or CalcType = "INTEGER") And DestType <> "BYTE" Then
		
		If ModePIC Then
			V1 = V1O: V2 = V2O
			AVH = GetByte(AV, 1)
			
			'V1 is word, get high byte
			If IsConst(V1) THEN
				CurrLine = LinkedListInsert(CurrLine, " movlw " + GetByte(V1, 1))
				V1H = GetCalcVar("BYTE")
				CurrLine = LinkedListInsert(CurrLine, " movwf " + V1H)
				FreeCalcVar V1
			Else
				V1H = GetByte(V1, 1)
			END IF
			...
			If ChipFamily = 12 Or ChipFamily = 14 Then
				'Put V2H into W
				Temp = " movf " + GetByte(V2, 1) + ",W"
				IF IsConst(V2) THEN Temp = " movlw " + GetByte(V2, 1)
				IF NOT IsConst(V2) and V2RType = "BYTE" THEN Temp = " clrw"
				...
				IF V1RType = "WORD" Or V1RType = "INTEGER" THEN
					If GetByte(V1, 1) <> AVH Then
						IF Act = "+" THEN CurrLine = LinkedListInsert(CurrLine, " addwf " + V1H + ",W")
						IF Act = "-" THEN CurrLine = LinkedListInsert(CurrLine, " subwf " + V1H + ",W")
						CurrLine = LinkedListInsert(CurrLine, " movwf " + AVH)
					Else
						IF Act = "+" THEN CurrLine = LinkedListInsert(CurrLine, " addwf " + V1H + ",F")
						IF Act = "-" THEN CurrLine = LinkedListInsert(CurrLine, " subwf " + V1H + ",F")
					End If
				Else
					If Act = "+" Then
						CurrLine = LinkedListInsert(CurrLine, " movwf " + AVH)
					Else
						CurrLine = LinkedListInsert(CurrLine, " clrf " + AVH)
						CurrLine = LinkedListInsert(CurrLine, " subwf " + AVH + ",F")
					End If
				END IF
			...
			Else
				Temp = " movf " + GetByte(V2, 1) + ",W"
				IF IsConst(V2) THEN Temp = " movlw " + GetByte(V2, 1)
				IF NOT IsConst(V2) and V2Type = "BYTE" THEN Temp = " clrw"
				CurrLine = LinkedListInsert(CurrLine, Temp)
				If GetByte(V1, 1) <> AVH Then
					IF V1RType = "WORD" Or V1RType = "INTEGER" THEN
						IF Act = "+" THEN CurrLine = LinkedListInsert(CurrLine, " addwfc " + V1H + ",W")
						IF Act = "-" THEN CurrLine = LinkedListInsert(CurrLine, " subwfb " + V1H + ",W")
					Else
						'normally, would add V1, W and C, and put in W
						'Or, subtract W and B from V1, and put in W 
						If Act = "+" Then
							'Need to add W and C, put in W
							CurrLine = LinkedListInsert(CurrLine, " btfsc STATUS,C")
							CurrLine = LinkedListInsert(CurrLine, " addlw 1")
						Else
							'Subtract W and B from 0, put in W
							CurrLine = LinkedListInsert(CurrLine, " btfss STATUS,C")
							CurrLine = LinkedListInsert(CurrLine, " addlw 1")
							CurrLine = LinkedListInsert(CurrLine, " sublw 0")
						End If
					END IF
					CurrLine = LinkedListInsert(CurrLine, " movwf " + AVH)
				
				Else
					'V1 will always be a word to end up here
					IF V1RType = "WORD" Or V1RType = "INTEGER" THEN
						IF Act = "+" THEN CurrLine = LinkedListInsert(CurrLine, " addwfc " + V1H + ",F")
						IF Act = "-" THEN CurrLine = LinkedListInsert(CurrLine, " subwfb " + V1H + ",F")
					END If
				End If
		...
		ElseIf ModeAVR Then
			'Need to do some processing differently depending on whether V1 or V2 are word/integer
			If V1Type = "WORD" Or V2Type = "INTEGER" Then
				'V1 and V2 are word/int, add/sub normally
				If V2Type = "WORD" Or V2Type = "INTEGER" Then
					If Act = "+" Then CurrLine = LinkedListInsert(CurrLine, " adc " + GetByte(R1, 1) + "," + GetByte(R2, 1))
					If Act = "-" Then CurrLine = LinkedListInsert(CurrLine, " sbc " + GetByte(R1, 1) + "," + GetByte(R2, 1))
				'V1 word, V2 byte, add carry
				Else
					CurrLine = LinkedListInsert(CurrLine, " brcc PC + 2")
					If Act = "+" Then CurrLine = LinkedListInsert(CurrLine, " inc " + GetByte(R1, 1))
					If Act = "-" Then CurrLine = LinkedListInsert(CurrLine, " dec " + GetByte(R1, 1))
				End If
				
			Else
				'V1 byte, V2 word, move V2 high into V1 high
				If V2Type = "WORD" Or V2Type = "INTEGER" Then
					CurrLine = LinkedListInsert(CurrLine, " mov " + GetByte(R1, 1) + "," + GetByte(R2, 1))
					If Act = "-" Then CurrLine = LinkedListInsert(CurrLine, " neg " + GetByte(R1, 1))
					CurrLine = LinkedListInsert(CurrLine, " brcc PC + 2")
					If Act = "+" Then CurrLine = LinkedListInsert(CurrLine, " inc " + GetByte(R1, 1))
					If Act = "-" Then CurrLine = LinkedListInsert(CurrLine, " dec " + GetByte(R1, 1))
				'V1 byte, V2 byte, clear R1
				Else
					CurrLine = LinkedListInsert(CurrLine, " clr " + GetByte(R1, 1))
				End If
			End If
...
FUNCTION CompileCalcCondition(OutList As CodeSection Pointer, V1 As String, Act As String, V2 As String, Origin As String, Answer As String) As String
	...
	If CalcType = "BIT" And IsConst(V2) Then
		...
		If ModePIC Then
			CurrLine = LinkedListInsert(CurrLine, " clrf SysCalcTempX")
			If TestFor = 1 Then
				CurrLine = LinkedListInsert(CurrLine, " btfsc " + BitName)
			Else
				CurrLine = LinkedListInsert(CurrLine, " btfss " + BitName)
			End If
			CurrLine = LinkedListInsert(CurrLine, " comf SysCalcTempX,F")
	...
	Else
		...
		AddVar "SysCalcTempA", CalcType, 1, Subroutine(SourceSub), "REAL", Origin
		AddVar "SysCalcTempB", CalcType, 1, Subroutine(SourceSub), "REAL", Origin
		AddVar "SysCalcTempX", CalcType, 1, Subroutine(SourceSub), "REAL", Origin
...
'		If Act = ">" Or Act = "}" Then
		...
		Else
			...
			CT2 = "SysCalcTempB"
		End If
		SetCalcTempType "A", CalcType
		SetCalcTempType "B", CalcType
...
FUNCTION CompileCalcLogic (OutList As CodeSection Pointer, V1 As String, Act As String, V2 As String, Origin As String, Answer As String) As String
	...
	Dim As String V1O, V2O, V1Type, V2Type, CalcType, DestType, AV, R1, R2, AVH
	Dim As String Temp, Cmd, Ovr
	Dim As Integer CD, SourceSub, DestSub
	Dim As LinkedListElement Pointer CurrLine, NewCode
	CurrLine = OutList->CodeEnd
	...
	If (Act <> "!" And IsConst(V1) And IsConst(V2)) OR (Act = "!" And IsConst(V2)) Then
		...
		IF Act = "!" THEN
			...
			If CalcType = "WORD" OR CalcType = "INTEGER" Then Return Trim(Str(NOT MakeDec(V1)))
		End If
	End If
	...
	If ModeAVR Then
		If Act <> "!" Then
			NewCode = LinkedListCreate
			R1 = PutInRegister(NewCode, V1, CalcType, Origin)
			AddVar V1, "BYTE", 1, Subroutine(SourceSub), "REAL", Origin
			CurrLine = LinkedListInsertList(CurrLine, NewCode)
		End If
	...
	'Shortcuts for V1 low = 0
	If Act <> "!" And IsConst(V1) And GetByte(V1, 0) = "0" And ModePIC Then
		If ModePIC Then AddVar AV, "BYTE", 1, 0, "REAL", Origin
		
		If Act = "&" Then
			CurrLine = LinkedListInsert(CurrLine, " clrf " + AV)
			Goto CalcLogicLowDone
		End If
		If Act = "|" Then
			CurrLine = LinkedListInsert(CurrLine, " movf " + GetByte(V2, 0) + ",W")
			CurrLine = LinkedListInsert(CurrLine, " movwf " + AV)
			
			Goto CalcLogicLowDone
		End If
		If Act = "#" Then
			CurrLine = LinkedListInsert(CurrLine, " comf " + GetByte(V2, 0) + ",W")
			CurrLine = LinkedListInsert(CurrLine, " movwf " + AV)
			Goto CalcLogicLowDone
		End If
	End If
	...
	If ModePIC Then
		'Move V1 into W
		IF Act <> "!" THEN
			Cmd = " movf " + GetByte(V1, 0) + ",W": IF IsConst(V1) THEN Cmd = " movlw " + GetByte(V1, 0)
			CurrLine = LinkedListInsert(CurrLine, Cmd)
			IF NOT IsConst(V1) THEN AddVar V1, "BYTE", 1, 0, "REAL", Origin
		END IF
	
		'AND/OR/XOR/NOT W with V2
		IF Act = "&" THEN Cmd = " andwf "
		IF Act = "|" THEN Cmd = " iorwf "
		IF Act = "#" THEN Cmd = " xorwf "
		IF Act = "!" THEN Cmd = " comf "
		AddVar V2, "BYTE", 1, 0, "REAL", Origin
		If AV <> V2 Then
			CurrLine = LinkedListInsert(CurrLine, Cmd + GetByte(V2, 0) + ",W")
			CurrLine = LinkedListInsert(CurrLine, " movwf " + AV)
		Else
			CurrLine = LinkedListInsert(CurrLine, Cmd + GetByte(V2, 0) + ",F")
		End If
		
		'Write W to answer
		AddVar AV, "BYTE", 1, 0, "REAL", Origin
	ElseIf ModeAVR Then
		IF Act = "&" THEN CurrLine = LinkedListInsert(CurrLine, " and " + R2 + "," + R1)
		IF Act = "|" THEN CurrLine = LinkedListInsert(CurrLine, " or " + R2 + "," + R1)
		IF Act = "#" THEN CurrLine = LinkedListInsert(CurrLine, " eor " + R2 + "," + R1)
		IF Act = "!" THEN CurrLine = LinkedListInsert(CurrLine, " com " + R2)
	End If
	CalcLogicLowDone:
	
	'Repeat for high byte
	IF CalcType = "WORD" Or CalcType = "INTEGER" THEN
		'Print "High: ", V1, Act, V2
		'Possible scenarios:
		'V1 V2
		'L  B
		'L  W
		...
		'NOT (Byte and Word)
		If Act = "!" Then
			If TypeOfVar(V2, Subroutine(GetSubID(Origin))) = "BYTE" Then
				If ModePIC Then
					If ChipFamily <> 16 Then
						CurrLine = LinkedListInsert(CurrLine, " movlw 255")
						CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(AV, 1))
					End If
					If ChipFamily = 16 Then CurrLine = LinkedListInsert(CurrLine, " setf " + GetByte(AV, 1))
				ElseIf ModeAVR Then
					CurrLine = LinkedListInsert(CurrLine, " clr " + GetByte(AV, 1))
					CurrLine = LinkedListInsert(CurrLine, " com " + GetByte(AV, 1))
				End If
			End If
			If TypeOfVar(V2, Subroutine(GetSubID(Origin))) <> "BYTE" Then
				If ModePIC Then
					CurrLine = LinkedListInsert(CurrLine, " comf " + GetByte(V2, 1) + ",W")
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(AV, 1))
				ElseIf ModeAVR Then
					CurrLine = LinkedListInsert(CurrLine, " com " + R2 + "_H")
				End If
			End If
			Goto CalcLogicHighDone
		End If
		
		'(Byte and Byte)
		If TypeOfVar(V1, Subroutine(GetSubID(Origin))) <> "WORD" And TypeOfVar(V2, Subroutine(GetSubID(Origin))) <> "WORD" AND Not IsConst(V1) AND Not IsConst(V2) Then
			If Act = "#" Then
				If ModePIC Then
					If ChipFamily <> 16 Then
						CurrLine = LinkedListInsert(CurrLine, " movlw 255")
				...
				ElseIf ModeAVR Then
					...
					CurrLine = LinkedListInsert(CurrLine, " com " + AV + "_H")
				End If
			Else
				CurrLine = LinkedListInsert(CurrLine, " clr " + AV + "_H")
			End If
		...
		'V2 word
		If TypeOfVar(V2, Subroutine(GetSubID(Origin))) = "WORD" Then
			'V1 byte
			'If TypeOfVar(V1, Subroutine(GetSubID(Origin))) <> "WORD" And Not IsConst(V1) Then
			If TypeOfVar(V1, Subroutine(GetSubID(Origin))) <> "WORD" Then
				If Act = "&" Then
					If ModePIC Then CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(AV, 1))
					If ModeAVR Then CurrLine = LinkedListInsert(CurrLine, " clr " + AV + "_H")
				End If
				If Act = "|" Or Act = "#" Then
					If ModePIC Then
						If ChipFamily <> 16 Then
							CurrLine = LinkedListInsert(CurrLine, " movf " + GetByte(V2, 1) + ",W")
							CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(AV, 1))
						End If
						If ChipFamily = 16 Then CurrLine = LinkedListInsert(CurrLine, " movff " + GetByte(V2, 1) + "," + GetByte(AV, 1))
					End If
				End If
				Goto CalcLogicHighDone
			End If
			...
			'V1 word or literal
			If ModePIC Then
				Cmd = " movf " + GetByte(V1, 1) + ",W"
				If IsConst(V1) Then Cmd = " movlw " + GetByte(V1, 1)
				CurrLine = LinkedListInsert(CurrLine, Cmd)
				IF Act = "&" THEN Cmd = " andwf "
				IF Act = "|" THEN Cmd = " iorwf "
				IF Act = "#" THEN Cmd = " xorwf "
				AddVar GetByte(V2, 1), "BYTE", 1, 0, "REAL", Origin
				If V2 <> AV Then
					CurrLine = LinkedListInsert(CurrLine, Cmd + GetByte(V2, 1) + ",W")
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(AV, 1))
				Else
					CurrLine = LinkedListInsert(CurrLine, Cmd + GetByte(V2, 1) + ",F")
				End If
				
			ElseIf ModeAVR Then
				IF Act = "&" THEN CurrLine = LinkedListInsert(CurrLine, " and " + GetByte(R2, 1) + "," + GetByte(R1, 1))
				IF Act = "|" THEN CurrLine = LinkedListInsert(CurrLine, " or " + GetByte(R2, 1) + "," + GetByte(R1, 1))
				IF Act = "#" THEN CurrLine = LinkedListInsert(CurrLine, " eor " + GetByte(R2, 1) + "," + GetByte(R1, 1))
			End If
			Goto CalcLogicHighDone
		End If
		...
		'V2 byte
		If TypeOfVar(V2, Subroutine(GetSubID(Origin))) <> "WORD" Then
			If Act = "&" Then CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(AV, 1))
			If Act = "|" Or Act = "#" Then
				Cmd = " movf " + GetByte(V1, 1) + ",W"
				If IsConst(V1) Then Cmd = " movlw " + GetByte(V1, 1)
				CurrLine = LinkedListInsert(CurrLine, Cmd)
				CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(AV, 1))
			End If
			'Print "V2 Byte", V1, Act, V2
			'Print , TempData(CSC - 1), TempData(CSC)
			Goto CalcLogicHighDone
		End If
		
		If ModePIC Then CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(AV, 1))
		If ModeAVR Then CurrLine = LinkedListInsert(CurrLine, " clr " + GetByte(AV, 1))
...
FUNCTION CompileCalcMult (OutList As CodeSection Pointer, V1 As String, Act As String, V2 As String, Origin As String, Answer As String) As String
	...
	AddVar "SysCalcTempA", "BYTE", 1, 0, "REAL", Origin
	AddVar "SysCalcTempB", "BYTE", 1, 0, "REAL", Origin
	AddVar "SysCalcTempX", "BYTE", 1, 0, "REAL", Origin
	...
	If CalcType = "BYTE" And Act = "*" And ModeAVR And HMult Then
		CurrLine = LinkedListInsert(CurrLine, " mul SysCalcTempA,SysCalcTempB")
		AV = "SysCalcTempX"
		Goto MultDivAnswer
	End If
	...
	'Write answer
	If Act = "*" Then AV = "SysCalcTempX"
	If Act = "/" Then AV = "SysCalcTempA"
	If Act = "%" Then AV = "SysCalcTempX"
	'Print V1, V2, AV, CalcType
	SetCalcTempType "SysCalcTempA", CalcType
	SetCalcTempType "SysCalcTempX", CalcType
...
Function CompileCalcUnary(OutList As CodeSection Pointer, Act As String, V2 As String, Origin As String, Answer As String) As String
	...
	Dim As Integer SourceSub, DestSub, CD
	Dim As LinkedListElement Pointer CurrLine, NewCode
	CurrLine = OutList->CodeEnd
	...
	DestType = TypeOfVar(Answer, Subroutine(DestSub))
	CalcType = V2Type
	If CastOrder(DestType) > CastOrder(CalcType) Then CalcType = DestType
	V2 = DelType(V2)
	...
	If IsConst(V2) And InStr(V2, "@") = 0 Then
		IF Act = "!" THEN
			If CalcType = "BYTE" Then Return Trim(Str((NOT MakeDec(V2)) AND 255))
			If CalcType = "WORD" OR CalcType = "INTEGER" Then Return Trim(Str(NOT MakeDec(V2)))
		End If
		If Act = "-" THEN
			'Do nothing
			Return "-" + V2
	...
	If ModePIC Then
		...
		IF V2 <> AV Then
			...
			AddVar AV, "BYTE", 1, 0, "REAL", Origin
		Else
			CurrLine = LinkedListInsert(CurrLine, " comf " + GetByte(V2, 0) + ",F")
		End If
		...
		If TypeOfVar(V2, Subroutine(GetSubID(Origin))) <> "BYTE" Then
			If ModePIC Then
				If AV <> V2 Then
					...
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(AV, 1))
				Else
					CurrLine = LinkedListInsert(CurrLine, " comf " + GetByte(V2, 1) + ",F")
				End If
		...
		Else
			If CondType = "BYTE" Then
				...
				ElseIf ModeAVR Then
					...
					CurrLine = LinkedListInsert(CurrLine, Cmd)
				ElseIf ModeZ8 Then
					Print "Fix CompileConditions for Z8!"
				End If
		...
		Else
			...
			If ModeAVR Then
				...
				ElseIf IsConst(V1) Then
					R1 = "SysCalcTempA"
					CurrLine = LinkedListInsert(CurrLine, " ldi SysCalcTempA," + V1)
				Else
					R1 = "SysCalcTempA"
			...
			IF OP = "<" OR OP = ">" THEN
				...
				If ModePIC Then
					...
					If IsConst(V1) Then
						CurrLine = LinkedListInsert(CurrLine, " sublw " + V1)
					Else
						CurrLine = LinkedListInsert(CurrLine, " subwf " + V1 + ",W")
					End If
...
Sub CompileDim (CurrSub As SubType Pointer)
	Dim As String VarName, VarType, VarAlias, Origin, InLine
	Dim As Integer PD, IsAlias, Si, CD, NewVarCount
	Dim As LinkedListElement Pointer CurrLine
	...
	Do While CurrLine <> 0
		...
		If Left(InLine, 4) = "DIM " THEN
			...
			For CV = 1 to NewVarCount
				...
				'Add var
				If IsAlias Then
					AddVar(InLine, VarType, Si, CurrSub, "ALIAS:" + VarAlias, Origin)
				Else
					AddVar(InLine, VarType, Si, CurrSub, "REAL", Origin)
				End If
			Next
		End If
	...
	Do While CurrLine <> 0
		...
		IF Left(InLine, 7) = "REPEAT " THEN
			...
			If RepNone = 0 Then
				...
				If CheckZero Then
					If ModePIC And Not RepWord Then
						CurrLine = LinkedListInsert(CurrLine, " btfsc STATUS,Z")
					Else
						If RepWord Then
					...
					ElseIf ModeAVR Then
						CurrLine = LinkedListInsert(CurrLine, " jmp SysRepeatLoopEnd" + Str(RPLC) + LabelEnd)
					End If
				End If
	...
	Do While CurrLine <> 0
		...
		IF Left(InLine, 7) = "ROTATE " THEN
			...
			If ModeAVR Then
				...
				Else
					CurrLine = LinkedListInsert(CurrLine, "lds SysCalcTempA," + VarName)
					If VarType = "WORD" Then CurrLine = LinkedListInsert(CurrLine, "lds SysCalcTempA_H," + VarName + "_H")
					RotateReg = "SysCalcTempA"
					OldType = 2
				End If
			End If
			...
			If (ChipFamily <> 16 OR VarType = "WORD") And Not UseC Then
				If ModePIC Then
					...
					/'
					CurrLine = LinkedListInsert(CurrLine, " bcf STATUS,C")
					If Direction = "RIGHT" Then CurrLine = LinkedListInsert(CurrLine, " btfsc " + VarName + ",0")
					If Direction = "LEFT" Then
						If VarType <> "WORD" Then CurrLine = LinkedListInsert(CurrLine, " btfsc " + VarName + ",7")
						If VarType = "WORD" Then CurrLine = LinkedListInsert(CurrLine, " btfsc " + VarName + "_H,7")
					End If
					CurrLine = LinkedListInsert(CurrLine, " bsf STATUS,C")
					'/
					If Direction = "RIGHT" Then CurrLine = LinkedListInsert(CurrLine, " rrf " + VarName + ",W")
					If Direction = "LEFT" Then
						If VarType <> "WORD" Then CurrLine = LinkedListInsert(CurrLine, " rlf " + VarName + ",W")
						If VarType = "WORD" Then CurrLine = LinkedListInsert(CurrLine, " rlf " + VarName + "_H,W")
					End If
				...
				ElseIf ModeAVR Then
					...
					If Direction = "LEFT" Then
						If VarType <> "WORD" Then CurrLine = LinkedListInsert(CurrLine, " sbrc " + RotateReg + ",7")
						If VarType = "WORD" Then CurrLine = LinkedListInsert(CurrLine, " sbrc " + RotateReg + "_H,7")
					End If
					CurrLine = LinkedListInsert(CurrLine, " sec")
				End If
			End If
			
			'Rotate high byte (right only)
			IF VarType = "WORD" AND Direction = "RIGHT" THEN
				If ModePIC Then
			...
			'Rotate high byte (left only)
			IF VarType = "WORD" AND Direction = "LEFT" THEN
				If ModePIC Then
			...
			If ModeAVR Then
				...
				ElseIf OldType = 2 Then
					...
					IF VarType = "WORD" Then CurrLine = LinkedListInsert(CurrLine, " sts " + VarName + "_H,SysCalcTempA_H")
					IF VarType <> "WORD" THEN AddVar VarName, "BYTE", 1, 0, "REAL", Origin
				End If
			End If
...
Function CompileSubCall (InCall As SubCallType Pointer) As LinkedListElement Pointer
	...
	Dim As Integer CD, C, F, L, S, AF, StringConstCount, RP, ParamIsFn, LocOfFn
	...
	With (*InCall)
		...
		Else
			...
			FOR CD = 1 TO .Params
				...
				IF INSTR((*.Called).Params(CD).Type, "$") <> 0 Or INSTR((*.Called).Params(CD).Type, "()") <> 0 Or (*.Called).Params(CD).Type = "STRING" THEN C = 3
				...
				IF C = 3 THEN
					...
					ElseIf ModeAVR Then
						If SourceArrayPtr->Pointer = "POINTER" Then
							...
							BeforePos = LinkedListInsert(BeforePos, " ldi SysValueCopy," + SourceArrayHandler + "_H")
...
Sub CompileSubCalls(CompSub As SubType Pointer)
	...
	Do While CurrLine <> 0
		...
		If Left(CurrLine->Value, 1) = Chr(31) Then
			CurrLine = LinkedListDelete(CurrLine)
		Else
...
Function CompileVarSet (SourceIn As String, Dest As String, Origin As String) As LinkedListElement Pointer
	...
	Dim As String LTemp, HTemp, Source
	Dim As Integer CurrentSub, DestSub
	Dim As Integer DestReg, DestIO, SourceReg, SourceIO, S, L, H, CD
	Dim As PinDirType Pointer CurrPinDir
	...
	If Source = Dest Then
		Return OutList
	End If
	'Print Subroutine(CurrentSub)->Name; " Var copy " + Source + "(" + SType + ") to " + Dest + "(" + DType + ")"
	...
	Case "BYTE":
		...
		'byte > byte, word > byte, integer > byte
		Case "BYTE", "WORD", "INTEGER":
			AddVar Source, SType, 1, Subroutine(CurrentSub), "REAL", Origin
			If ModePIC Then
				If ChipFamily = 16 Then
					CurrLine = LinkedListInsert(CurrLine, " movff " + Source + "," + Dest)
				Else
					CurrLine = LinkedListInsert(CurrLine, " movf " + Source + ",W")
					CurrLine = LinkedListInsert(CurrLine, " movwf " + Dest)
				End If
			ElseIf ModeAVR Then
				If DestReg Then
					If SourceReg Then
						CurrLine = LinkedListInsert(CurrLine, " mov " + Dest + "," + Source)
					ElseIf SourceIO Then
						CurrLine = LinkedListInsert(CurrLine, " in " + Dest + "," + Source)
					Else
						CurrLine = LinkedListInsert(CurrLine, " lds " + Dest + "," + Source)
					End If
				ElseIf DestIO Then
					If SourceReg Then
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + "," + Source)
					ElseIf SourceIO Then
						CurrLine = LinkedListInsert(CurrLine, " in SysValueCopy," + Source)
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + ",SysValueCopy")
					Else
						CurrLine = LinkedListInsert(CurrLine, " lds SysValueCopy," + Source)
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + ",SysValueCopy")
					End If
				Else
					If SourceReg Then
						CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + "," + Source)
					ElseIf SourceIO Then
						CurrLine = LinkedListInsert(CurrLine, " in SysValueCopy," + Source)
						CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + ",SysValueCopy")
					Else
						CurrLine = LinkedListInsert(CurrLine, " lds SysValueCopy," + Source)
						CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + ",SysValueCopy")
					End If
			...
			ElseIf ModeZ8 Then
				CurrLine = LinkedListInsert(CurrLine, " ldx " + Dest + "," + Source)
...
'			If Left(Source, 1) = "@" Then
			If Instr(Source, "@") <> 0 Then
				S = 1
				'CSource = Mid(Source, 2)
				CSource = Source
				Replace CSource, "@", ""
			Else
				S = MakeDec(Source)
				IF S > 255 Then
					If AllowOverflow = 1 Then
						S = S MOD 256
					Else
						Temp = Message("BadValueType")
						Replace Temp, "%value%", Source
			...
			If ModePIC Then
				IF S > 0 Then
					CurrLine = LinkedListInsert(CurrLine, " movlw " + CSource)
					CurrLine = LinkedListInsert(CurrLine, " movwf " + Dest)
				Else
					CurrLine = LinkedListInsert(CurrLine, " clrf " + Dest)
				End If
			ElseIf ModeAVR Then
				CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + CSource)
				If DestReg Then
					CurrLine = LinkedListInsert(CurrLine, " mov " + Dest + ",SysValueCopy")
				ElseIf DestIO Then
					CurrLine = LinkedListInsert(CurrLine, " out " + Dest + ",SysValueCopy")
				Else
					CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + ",SysValueCopy")
	...
	Case "WORD":
		...
		Case "BYTE":
			...
			ElseIf ModeAVR Then
				...
				Else
					...
					End If
					CurrLine = LinkedListInsert(CurrLine, " clr SysValueCopy")
					CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + "_H,SysValueCopy")
				End If
			End If
		...
		Case "WORD", "INTEGER":
			If ModePIC Then
				If ChipFamily = 16 Then
					CurrLine = LinkedListInsert(CurrLine, " movff " + Source + "_H," + Dest + "_H")
					CurrLine = LinkedListInsert(CurrLine, " movff " + Source + "," + Dest)
				Else
					CurrLine = LinkedListInsert(CurrLine, " movf " + Source + "_H,W")
					CurrLine = LinkedListInsert(CurrLine, " movwf " + Dest + "_H")
					CurrLine = LinkedListInsert(CurrLine, " movf " + Source + ",W")
					CurrLine = LinkedListInsert(CurrLine, " movwf " + Dest)
				End If
			ElseIf ModeAVR Then
				If DestReg Then
					If SourceReg Then
						CurrLine = LinkedListInsert(CurrLine, " mov " + Dest + "," + Source)
						CurrLine = LinkedListInsert(CurrLine, " mov " + Dest + "_H," + Source + "_H")
					ElseIf SourceIO Then
						CurrLine = LinkedListInsert(CurrLine, " in " + Dest + "," + Source)
						CurrLine = LinkedListInsert(CurrLine, " in " + Dest + "_H," + Source + "_H")
					Else
						CurrLine = LinkedListInsert(CurrLine, " lds " + Dest + "," + Source)
						CurrLine = LinkedListInsert(CurrLine, " lds " + Dest + "_H," + Source + "_H")
					End If
				ElseIf DestIO Then
					If SourceReg Then
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + "_H," + Source + "_H")
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + "," + Source)
					ElseIf SourceIO Then
						CurrLine = LinkedListInsert(CurrLine, " in SysValueCopy," + Source + "_H")
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + "_H,SysValueCopy")
						CurrLine = LinkedListInsert(CurrLine, " in SysValueCopy," + Source)
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + ",SysValueCopy")
					Else
						CurrLine = LinkedListInsert(CurrLine, " lds SysValueCopy," + Source + "_H")
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + "_H,SysValueCopy")
						CurrLine = LinkedListInsert(CurrLine, " lds SysValueCopy," + Source)
						CurrLine = LinkedListInsert(CurrLine, " out " + Dest + ",SysValueCopy")
					End If
				Else
					If SourceReg Then
						CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + "_H," + Source + "_H")
						CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + "," + Source)
					ElseIf SourceIO Then
						CurrLine = LinkedListInsert(CurrLine, " in SysValueCopy," + Source + "_H")
						CurrLine = LinkedListInsert(CurrLine, " sts " + Dest + "_H,SysValueCopy")
		...
		Case "CONST":
			If Left(Source, 1) = "@" Then
				If ModePIC Then
					LTemp = "low " + Mid(Source, 2)
					HTemp = "high " + Mid(Source, 2)
				ElseIf ModeAVR Then
					LTemp = "low(" + Mid(Source, 2) + ")"
					HTemp = "high(" + Mid(Source, 2) + ")"
				End If
				L = 1: H = 1
			Else
				L = VAL(GetByte(Source, 0))
				H = VAL(GetByte(Source, 1))
				LTemp = Str(L)
				HTemp = Str(H)
			End If
			If ModePIC Then
				If L > 0 Then
					CurrLine = LinkedListInsert(CurrLine, " movlw " + LTemp)
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(Dest, 0))
				Else
					CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(Dest, 0))
				End If
				If H > 0 Then
					CurrLine = LinkedListInsert(CurrLine, " movlw " + HTemp)
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(Dest, 1))
				Else
					CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(Dest, 1))
				End If
			ElseIf ModeAVR Then
				If DestReg Then
					CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + LTemp)
					CurrLine = LinkedListInsert(CurrLine, " mov " + GetByte(Dest, 0) + ",SysValueCopy")
					CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + HTemp)
					CurrLine = LinkedListInsert(CurrLine, " mov " + GetByte(Dest, 1) + ",SysValueCopy")
				Else
					CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + LTemp)
					CurrLine = LinkedListInsert(CurrLine, " sts " + GetByte(Dest, 0) + ",SysValueCopy")
					CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + HTemp)
					CurrLine = LinkedListInsert(CurrLine, " sts " + GetByte(Dest, 1) + ",SysValueCopy")
				End If
			End If
	...
	Case "INTEGER":
		...
		'const > integer
		Case "CONST":
			L = MakeTwosComp(Source, 0)
			H = MakeTwosComp(Source, 1)
			If ModePIC Then
				If L > 0 Then
					CurrLine = LinkedListInsert(CurrLine, " movlw " + Str(L))
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(Dest, 0))
				Else
					CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(Dest, 0))
				End If
				If H > 0 Then
					CurrLine = LinkedListInsert(CurrLine, " movlw " + Str(H))
					CurrLine = LinkedListInsert(CurrLine, " movwf " + GetByte(Dest, 1))
				Else
					CurrLine = LinkedListInsert(CurrLine, " clrf " + GetByte(Dest, 1))
				End If
			ElseIf ModeAVR Then
				CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + Str(L))
				CurrLine = LinkedListInsert(CurrLine, " sts " + GetByte(Dest, 0) + ",SysValueCopy")
				CurrLine = LinkedListInsert(CurrLine, " ldi SysValueCopy," + Str(H))
				CurrLine = LinkedListInsert(CurrLine, " sts " + GetByte(Dest, 1) + ",SysValueCopy")
			End If
			...
			If IsCalc(SourceData) Then
				...
				Do While FindLine <> 0
					...
					IF INSTR(Temp, "SYSASMCALC") <> 0 Then
						T = VAL(Mid(Temp, INSTR(Temp, "SYSASMCALC") + 11))
...
Function ConfigValueMatch(ConfigIn As String, ConfigValueIn As String) As Integer
	...
	Dim As String Config, ConfigValue
	Config = ConfigIn
	...
	If ConfigValue = "INT" Or ConfigValue = "INT IO" Then
		...
		If InStr(Config, "INTOSC") <> 0 Then Return -1
...
Sub ExtAssembler
	...
	AsmExe = ReplaceToolVariables(AsmExe)
	AsmParams = ReplaceToolVariables(AsmParams, "asm")
...
Function GetRegisterLoc(RegName As String) As Integer
	...
	If ModePIC Then
		...
		Select Case RegNameTidy
			...
			Case "syscalctempx", "delaytemp": DestLoc = 0
			Case "syscalctempx_h", "delaytemp2": DestLoc = 1
			Case "syscalctempx_u", "sysdivmultx", "syswaittempms", "sysstringb": DestLoc = 2
			Case "syscalctempx_e", "sysdivmultx_h", "syswaittempms_h", "sysstringb_h": DestLoc = 3
			Case "sysdivloop", "syswaittemp10ms", "syswaittemps": DestLoc = 4
			Case "syscalctempa", "syswaittempus", "syswaittemp10us", "syswaittempm": DestLoc = 5
			Case "syscalctempa_h", "sysstringlength", "syswaittempus_h", "syswaittemph": DestLoc = 6
			Case "syscalctempa_u", "sysdivmulta", "sysstringa": DestLoc = 7
			Case "syscalctempa_e", "sysdivmulta_h", "sysstringa_h": DestLoc = 8	
			Case "syscalctempb": DestLoc = 9
			Case "syscalctempb_h": DestLoc = 10
			Case "syscalctempb_u", "sysdivmultb", "sysreada": DestLoc = 11
			Case "syscalctempb_e", "sysdivmultb_h", "sysreada_h": DestLoc = 12
	...
	ElseIf ModeAVR Then
		...
		RegNameTidy = LCase(Trim(RegName))
		Select Case RegNameTidy
			Case "syswaittempus": DestLoc = 27
			Case "syswaittempus_h": DestLoc = 28
			Case "syswaittemp10us": DestLoc = 27
			Case "syswaittempms": DestLoc = 29
			Case "syswaittempms_h": DestLoc = 30
			Case "syswaittemp10ms": DestLoc = 31
			Case "syswaittemps": DestLoc = 31
			Case "syswaittempm": DestLoc = 27
			Case "syswaittemph": DestLoc = 28
			Case "delaytemp": DestLoc = 25
			Case "delaytemp2": DestLoc = 26
			Case "syscalctempx": DestLoc = 0
			Case "syscalctempx_h": DestLoc = 1
			Case "syscalctempx_u": DestLoc = 2
			Case "syscalctempx_e": DestLoc = 3
			Case "syscalctempa": DestLoc = 24
			Case "syscalctempa_h": DestLoc = 25
			Case "syscalctempa_u": DestLoc = 26
			Case "syscalctempa_e": DestLoc = 27
			Case "syscalctempb": DestLoc = 28
			Case "syscalctempb_h": DestLoc = 29
			Case "syscalctempb_u": DestLoc = 30
			Case "syscalctempb_e": DestLoc = 31
			Case "sysdivmulta": DestLoc = 26
			Case "sysdivmulta_h": DestLoc = 27
			Case "sysdivmultb": DestLoc = 30
			Case "sysdivmultb_h": DestLoc = 31
			Case "sysdivmultx": DestLoc = 2
			Case "sysdivmultx_h": DestLoc = 3
			Case "sysdivloop": DestLoc = 23
			Case "syssignbyte": DestLoc = 4
			Case "sysvaluecopy": DestLoc = 22
			Case "sysstringlength": DestLoc = 25
			Case "sysstringa": DestLoc = 26
			Case "sysstringa_h": DestLoc = 27
			Case "sysstringb": DestLoc = 28
			Case "sysstringb_h": DestLoc = 29
			Case "sysreada": DestLoc = 30
			Case "sysreada_h": DestLoc = 31
		End Select
	End If
		...
		For PD = 1 to .ParamCount
			With .Params(PD)
				...
				Select Case UCASE(.Type)
					Case "BIT", "BYTE", "WORD", "INTEGER", "FLOAT", "STRING": NewData = UCase(.Type) + ":"
				End Select
...
Function GetSubParam (ParamIn As String) As SubParam
...
SUB InitCompiler
	...
	Dim As Integer AsmNotSet, ProgNotSet, OutNotSet, PresNotSet, VbsNotSet, PauseNotSet
	...
	Do While COMMAND(CD) <> ""
		...
		'(Alters the error listing format, removes some contact info from asm)
	...
	For FindTool = 1 To ToolCount
		With Tool(FindTool)
			If .Name = LCase(AsmExe) Then
				AsmExe = ReplaceToolVariables(.Cmd)
				AsmParams = .Params
			ElseIf .Name = LCase(PrgExe) Then
				PrgExe = ReplaceToolVariables(.Cmd)
				PrgParams = .Params
				PrgDir = .WorkingDir
			End If
		End With
...
Function IsIOReg (RegNameIn As String) As Integer
	...
	For SD = 1 to SVC
		If SysVars(SD, 1) = RegName Then
			If ModePIC Then
				Return -1
			ElseIf ModeAVR Then
				If Val(SysVars(SD, 2)) > 63 Then
					Return 0
				Else
					Return -1
				End If
...
SUB OptimiseIF
	...
	Do While CurrLine <> 0
		...
		IF Left(Temp, 5) = "ENDIF" THEN
			...
			If ModePIC Then
				'IF PL(2)->Value = " goto " + Temp And Left(PL(1)->Value, 6) <> " call " THEN
				'Note 26/7/2009: Not sure why call wasn't allowed? Remnant of old lcall code?
...
SUB ProcessArrays (CompSub As SubType Pointer)
	...
	Do While CurrLine <> 0
		...
		If ModeAVR And (Instr(InLine, "low(") <> 0 Or Instr(InLine, "high(") <> 0) Then Goto CompileArraysNextLine
		...
		IF AF <> 0 THEN
			...
			Else
				...
				IF Variable(AF)->Pointer = "POINTER" THEN
					ArrayHandler = "Sys" + ArrayName + "Handler"
					AddVar ArrayHandler, "WORD", 1, 0, "REAL", Origin
					ArrayPointer = -1
				END IF
				...
				If ModePIC And (ChipFamily = 12 Or ChipFamily = 14) Then
					If ArrayPointer = 0 Then
						NewCodeLine = LinkedListInsert(NewCodeLine, "FSR = [byte]" + ArrayHandler + "+" + ArrayPosition)
						If HighFSR Then
							NewCodeLine = LinkedListInsert(NewCodeLine, " bankisel " + ArrayName)
						End If
					Else
						NewCodeLine = LinkedListInsert(NewCodeLine, "FSR = " + ArrayHandler + "+" + ArrayPosition)
						If HighFSR Then
				...
				ElseIf ModePIC And ChipFamily = 15 Then
					NewCodeLine = LinkedListInsert(NewCodeLine, "AFSR0 = " + ArrayHandler + "+" + ArrayPosition)
					If UseTempVar Then
				...
				ElseIf ModeAVR Then
					AddVar "SysStringA", "WORD", 1, 0, "REAL", Origin
					If ArrayPointer = 0 Then
						NewCodeLine = LinkedListInsert(NewCodeLine, "SysStringA = @" + ArrayName + "+" + ArrayPosition)
					Else
						NewCodeLine = LinkedListInsert(NewCodeLine, "SysStringA = " + ArrayHandler + "+" + ArrayPosition)
					End If
					If UseTempVar Then
...
#ENDIF
	...
	DO WHILE NOT EOF(1)
		...
		ElseIf ReadDataMode = "[registers]" AND INSTR(InLine, ",") <> 0 THEN
			SVC = SVC + 1
			InLine = UCase(InLine)
			SysVars(SVC, 1) = Trim(Left(InLine, INSTR(InLine, ",") - 1))
			SysVars(SVC, 2) = Trim(Mid(InLine, INSTR(InLine, ",") + 1))
		...
		ElseIf ReadDataMode = "[bits]" AND INSTR(InLine, ",") <> 0 THEN
			SVBC = SVBC + 1
			InLine = UCase(InLine)
			SysVarBits(SVBC, 1) = Trim(Left(InLine, INSTR(InLine, ",") - 1)) 'Name
			TempData = Trim(Mid(InLine, INSTR(InLine, ",") + 1))
			SysVarBits(SVBC, 3) = Trim(Left(TempData, INSTR(TempData, ",") - 1)) 'Location
			SysVarBits(SVBC, 2) = Trim(Mid(TempData, INSTR(TempData, ",") + 1)) 'Parent
...
Sub RecordSubCall(CompSub As SubType Pointer, CalledSub As SubType Pointer)
	...
	With *CompSub
		...
		Do While FindCall <> 0
			If FindCall->MetaData = CalledSub Then Exit Sub
			FindCall = FindCall->Next
		Loop
		
		'Sub not in list, add it
		'.Call(.Calls) = CalledSub
		LinkedListInsert(.CallList, CalledSub)
	End With
...
Function RequestSub(Requester As SubType Pointer, SubNameIn As String, SubSigIn As String = "") As Integer
	...
	'If Requester is 0 then no call will be logged
...
Sub RetrySubRequests
	'Check AttemptedCallList for calls that could not be record
...
Sub WriteAssembly
	...
	ElseIf ModeAVR Then
		...
		For FindSREG = 1 To SVBC
			If UCASE(SysVarBits(FindSREG, 3)) = "SREG" Then
				Print #1, "#define " + SysVarBits(FindSREG, 1) + " " + SysVarBits(FindSREG, 2) 
			End If
		Next
	...
	ElseIf ModeZ8 Then
		...
		For PD = 1 To SVC
			...
			If AddSFR Then Print #1, AsmTidy(SysVars(PD, 1) + " EQU " + SysVars(PD, 2)) 
		Next
		Print #1, ""
		Print #1, ";Ram end"
...
Sub MergeSubroutines
	...
	Dim As Integer SubsFit, FirstPageFull, ForceFit
		...
		Do
			...
			For CurrSub = 0 To SBC
				If Subroutine(CurrSub)->Required And Subroutine(CurrSub)->FirstPage Then
					...
					ProgMemPage(1).CodeSize += Subroutine(CurrSub)->HexSize
			...
			For CurrSub = 1 To SubQueueCount
				...
				For CurrPage = 1 To ProgMemPages
					With ProgMemPage(CurrPage)
						If .MaxSize >= .CodeSize + CurrSubPtr->HexSize Then
	...
	For CurrPage = 1 To ProgMemPages
		...
		If ModePIC Then
			If ChipFamily = 16 Or IntSub = 0 Then
		...
		For CurrSub = 1 To SubQueueCount
			...
			If CurrSubPtr->Required And CurrSubPtr->LocationSet And CurrSubPtr->DestPage = CurrPage Then
				...
				End If
				'Print SubNameOut, CurrSubPtr->HexSize
	...
	For CD = 1 To TCC
		...
		ElseIf IsConst(ValueTemp) Then
			...
			Else
				...
				If TempValue = 0 Or TempValue = 1 Then ThisType = "BIT"
				If TempValue >= 2 And TempValue <= 255 Then ThisType = "BYTE"
				If TempValue >= 256 And TempValue <= 65535 Then ThisType = "WORD"
				If TempValue >= -32768 And TempValue <= -1 Then ThisType = "INTEGER"
